...
export class AiService {
  ...
  async summarizeTicket(ticketId: string) {
    ...
    const summarizer = new TicketSummarizerService(this.supabase, this.orgId || '')
  ...
  async generateDraft(ticketId: string, userId: string) {
    ...
    const autoResponder = new AutoResponderService(this.supabase, this.orgId || '')
    const thread = {
      id: ticket.id,
      ticket_id: ticket.id,
      messages: [], // you could fetch conversation or "ticket_replies"
    }
    const draftResponse = await autoResponder.generateDraftResponse(thread as any, ticketId)
